# CogniSys Phases 3-5 Implementation Guide

**Status**: Deferred (implement after Phases 1-2 complete)
**Prerequisites**:
- ✅ Phase 1: `.CogniSys/` structure and database created
...
#### 3.1 Implement `CogniSys register`

**File**: `CogniSys/commands/register.py`
...
#### 3.2 Implement `CogniSys classify`

**File**: `CogniSys/commands/classify.py`
...
def classify_pending_files(db_path, ml_model_path, confidence_threshold=0.70):
    ...
    """
    from CogniSys.ml.classifier import MLClassifier
...
#### 3.3 Implement `CogniSys organize`

**File**: `CogniSys/commands/organize.py`
...
# Verify
CogniSys status <file_path>      # Show file's journey
sqlite3 .CogniSys/file_registry.db "SELECT * FROM move_history LIMIT 5"
```
...
#### 4.1 Implement `CogniSys correct`

**File**: `CogniSys/commands/correct.py`
...
#### 4.2 Implement `CogniSys metrics`

**File**: `CogniSys/metrics.py`
...
metrics = CogniSysMetrics('.CogniSys/file_registry.db')
...
#### 5.1 Implement `CogniSys reorg`

**File**: `CogniSys/commands/reorg.py`
...
```bash
# Run reorganization
CogniSys reorg --dry-run
CogniSys reorg --execute

# Run again - should show "No changes needed"
CogniSys reorg --dry-run

# Verify stability metric
CogniSys metrics --stability
```
...
vim .CogniSys/config.yml  # Change a path template
...
### Phase 3 Complete:
- [ ] `CogniSys register` scans drop directory and registers files
- [ ] `CogniSys classify` uses ML model with 88.6% accuracy
- [ ] `CogniSys organize` moves files to canonical locations
- [ ] Database has complete audit trail (move_history populated)

### Phase 4 Complete:
- [ ] `CogniSys correct` logs manual corrections
- [ ] `CogniSys metrics` generates accuracy report
- [ ] Metrics show >85% accuracy
- [ ] Daily metrics automated (cron/Task Scheduler)

### Phase 5 Complete:
- [ ] `CogniSys reorg` works idempotently (run twice, no changes second time)
...
CogniSys register --scan-drop
CogniSys classify --confidence-threshold 0.70
CogniSys organize --dry-run
CogniSys organize --execute

# Phase 4: Accuracy Tracking
CogniSys correct <file_path> --type <doc_type> --reason "..."
CogniSys metrics --today
CogniSys metrics --report .CogniSys/logs/metrics_$(date +%Y-%m-%d).json
...
1. **Phase 3.1**: `CogniSys register` (files enter system)
2. **Phase 3.2**: `CogniSys classify` (ML classification)
3. **Phase 3.3**: `CogniSys organize` (move to canonical)
4. **Phase 4.1**: `CogniSys correct` (user feedback)
5. **Phase 4.2**: `CogniSys metrics` (accuracy measurement)
6. **Phase 5.1**: `CogniSys reorg` (idempotent refinement)
