# IFMOS ML Workflow Guide

Complete guide for training and deploying the IFMOS ML document classification system.

## 📋 Quick Start

```powershell
# Check system status
.\ifmos_ml_workflow.ps1

# Run complete workflow (all stages)
.\ifmos_ml_workflow.ps1 -Stage Complete
```
...
```powershell
.\ifmos_ml_workflow.ps1 -Stage Setup
```
...
```powershell
.\ifmos_ml_workflow.ps1 -Stage Process
```
...
```powershell
.\ifmos_ml_workflow.ps1 -Stage Collect
```
...
```powershell
# Check if ready
.\ifmos_ml_workflow.ps1 -Stage Train
...
|--------|---------|-------|
| `ifmos_ml_workflow.ps1` | Master workflow manager | `.\ifmos_ml_workflow.ps1 -Stage <Stage>` |
...
```powershell
Import-Module .\IFMOS-ML-Bridge.psm1
Get-IFMOSMLStats | Format-List
```

### View Categories

```powershell
Get-IFMOSMLCategories | Format-Table
```
...
$result = Invoke-IFMOSMLProcess -FilePath "C:\new_doc.pdf"
...
1. **Run the complete workflow**: `.\ifmos_ml_workflow.ps1 -Stage Complete`
2. **Monitor accuracy** as you collect more feedback
3. **Integrate with main IFMOS** PowerShell scripts
