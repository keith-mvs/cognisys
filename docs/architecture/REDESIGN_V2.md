# CogniSys Architecture Redesign - Drop-to-Canonical Pipeline
...
This document redesigns CogniSys to solve three critical problems:
...
- **Idempotent operations** - Re-running CogniSys refines in-place, doesn't duplicate
...
CREATE TABLE manual_corrections (
    ...
    file_id INTEGER NOT NULL,
    wrong_type TEXT,                      -- What CogniSys said
...
```
$PROJECT_ROOT/
├── .CogniSys/                              ← CogniSys state directory
│   ├── file_registry.db                 ← Main provenance database
│   ├── config.yml                       ← CogniSys configuration
│   └── logs/
...
**Configuration**: `.CogniSys/config.yml`
...
```bash
# Initialize CogniSys in a directory
CogniSys init --drop ~/00_Inbox --canonical ~/Documents/Organized_Canonical

# Register files from drop zone
CogniSys register --scan-drop

# Classify and move to canonical locations
CogniSys organize --dry-run               # Preview changes
CogniSys organize --execute               # Actually move files

# Query file status
CogniSys status <file_path>               # Show file's current state
CogniSys history <file_path>              # Show move history

# Manual correction (trains accuracy metrics)
CogniSys correct <file_id> --type financial_invoice --reason "wrong classification"

# Metrics
CogniSys metrics --today                  # Today's accuracy metrics
CogniSys metrics --report accuracy.json   # Export metrics
```
...
**What happens on each run**:
1. CogniSys scans `Organized_Canonical/`
...
def create_snapshot(canonical_root):
    ...
    """
    snapshot_dir = Path('.CogniSys/snapshots') / datetime.now().strftime('%Y%m%d_%H%M%S')
...
CogniSys_DB=".CogniSys/file_registry.db"

echo "=== CogniSys Consolidation Migration ==="
...
CogniSys organize --consolidate --dry-run  # Preview
read -p "Looks good? Execute? [y/N] " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    CogniSys organize --consolidate --execute
fi

# Step 5: Report duplicates
echo ""
echo "=== Duplicate Files (NOT moved) ==="
sqlite3 "$CogniSys_DB" -column <<SQL
SELECT
...
echo ""
echo "Verify first:"
echo "  CogniSys metrics --report migration_report.json"
```
...
### 3.1 Defining "Accuracy" for CogniSys
...
```python
class CogniSysMetrics:
    """Compute and track CogniSys accuracy metrics"""
    ...
    def auto_classification_accuracy(self):
        """
        % of files that CogniSys classified correctly without manual intervention.
...
**Daily Move Log Format** (`.CogniSys/logs/moves_YYYY-MM-DD.log`):
...
**Metrics Snapshot** (`.CogniSys/logs/metrics_YYYY-MM-DD.json`):
...
```bash
#!/bin/bash
# CogniSys_daily_metrics.sh
# Run this daily to track accuracy trends

CogniSys_DB=".CogniSys/file_registry.db"
TODAY=$(date +%Y-%m-%d)
METRICS_FILE=".CogniSys/logs/metrics_$TODAY.json"

# Generate today's metrics
./venv/Scripts/python.exe -c "
from CogniSys.metrics import CogniSysMetrics
metrics = CogniSysMetrics('$CogniSys_DB')
...
└────────┬────────┘
         │
         │ 1. CogniSys register --scan-drop
         ▼
┌─────────────────────────────────────────┐
│  .CogniSys/file_registry.db                │
│  - Register files                       │
...
└─────────────────┬───────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────┐
│  CogniSys correct <file> --type <new_type>│
│  → Logs correction to database          │
│  → Retrains model (optional)            │
└─────────────────┬───────────────────────┘
                  │
                  │ 5. CogniSys reorg (idempotent)
                  ▼
         (Files move within
         Organized_Canonical/
         no duplication)
...
# Classify them
CogniSys classify --confidence-threshold 0.70
# Output: Classified 47 files (3 low confidence, flagged for review)

# Review classification
CogniSys list --flagged
# Shows 3 files needing manual review

# Organize into canonical tree
CogniSys organize --dry-run  # Preview
CogniSys organize --execute  # Actually move

# Check metrics
CogniSys metrics --today
# Output:
...
CogniSys correct ~/Documents/Organized_Canonical/Financial/Invoices/2025/11/receipt.pdf \
...
# ... edit .CogniSys/config.yml to refine rules ...

CogniSys reorg --dry-run      # See what would move
CogniSys reorg --execute      # Moves files in-place, no duplication

# Generate weekly report
CogniSys metrics --report weekly_report_2025-11-28.json

# View accuracy trends
CogniSys metrics --trend --days 7
...
### Script 1: `CogniSys register` - Register Files from Drop Zone

```bash
#!/bin/bash
# CogniSys-register.sh

CogniSys_DB=".CogniSys/file_registry.db"
DROP_DIR=$(yq eval '.CogniSys.drop_directory' .CogniSys/config.yml)
...
find "$DROP_DIR" -type f | while read filepath; do
    ...
    existing=$(sqlite3 "$CogniSys_DB" "SELECT file_id FROM file_registry WHERE content_hash='$hash'")
...
### Script 2: `CogniSys classify` - Run ML Classification

```bash
#!/bin/bash
# CogniSys-classify.sh

CogniSys_DB=".CogniSys/file_registry.db"
ML_MODEL=$(yq eval '.CogniSys.ml_model_path' .CogniSys/config.yml)
THRESHOLD=$(yq eval '.CogniSys.confidence_threshold' .CogniSys/config.yml)

echo "Classifying pending files (threshold: $THRESHOLD)..."

# Get files pending classification
sqlite3 -separator $'\t' "$CogniSys_DB" \
...
from CogniSys.classifiers import MLClassifier
...
### Script 3: `CogniSys organize` - Move Files to Canonical

```bash
#!/bin/bash
# CogniSys-organize.sh

CogniSys_DB=".CogniSys/file_registry.db"
CANONICAL_ROOT=$(yq eval '.CogniSys.canonical_root' .CogniSys/config.yml)
DRY_RUN=false
...
SQL
        ...
        echo "$(date -Iseconds)|MOVE|file_id=$file_id|from=$filepath|to=$target_path|reason=initial|doc_type=$doc_type" \
            >> ".CogniSys/logs/moves_$(date +%Y-%m-%d).log"
    fi
done
...
### Script 4: `CogniSys metrics` - Compute Accuracy Metrics
...
echo "=== CogniSys Accuracy Metrics ==="
...
sqlite3 -column "$CogniSys_DB" <<SQL
SELECT
...
- [ ] Initialize `.CogniSys/` directory structure
- [ ] Create `file_registry.db` with schema
- [ ] Write `CogniSys init` command
- [ ] Write `CogniSys register` command
...
- [ ] Implement `CogniSys classify` with ML model
- [ ] Implement `CogniSys organize` with templates
- [ ] Test idempotent reorganization
- [ ] Validate template filling works

### Phase 4: Accuracy Tracking (Week 4)

- [ ] Implement `CogniSys correct` for manual corrections
- [ ] Implement `CogniSys metrics` reporter
- [ ] Set up daily metrics cron job
- [ ] Create accuracy dashboard (optional web UI)

### Phase 5: Production & Iteration (Ongoing)

- [ ] Use CogniSys daily for new files
...
| Problem | Solution |
|---------|----------|
| **Drop → Target linking unclear** | `.CogniSys/file_registry.db` tracks every file's journey |
...
2. I'll implement the core scripts (`CogniSys init`, `register`, `classify`, `organize`, `metrics`)
3. Run consolidation migration to merge existing trees
4. Set up daily metrics tracking
5. Iterate based on accuracy feedback

This architecture transforms CogniSys from a one-shot organizer into a **measurable, improvable system** with clear accuracy tracking and zero duplication cruft.
